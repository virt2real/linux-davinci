...
static int write_edid(struct v4l2_subdev *sd){
	int err = 0;
	int i = 0;
/* edid generated in the AW EDID Editor and Advantiv EDID Editor */

/* low profile version - support CEA standarts
 * 34 1920 x 1080 30Hz 74.25MHz
 * 33 1920 x 1080 25Hz 74.25MHz
 * 32 1920 x 1080 24Hz 74.25MHz
 */
/*
u8 edid_data[] = {
...
/* 1080P version - support CEA standarts
 * 16 1920 x 1080 60Hz 148.50MHz
 * 31 1920 x 1080 50Hz 158.50MHz
 * 34 1920 x 1080 30Hz 74.25MHz
 * 33 1920 x 1080 25Hz 74.25MHz
 * 32 1920 x 1080 24Hz 74.25MHz
 */
...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x98
	...
	u8 ksv_i2caddr = channel->KSV_ADDR;
	u8 edid_i2caddr = channel->EDID_ADDR;
...
static int adv7611_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
	case ADV7611_V4L2_CONTROL_OUTPUT_FORMAT:
		ctrl->value = channel->output_format;
		break;
	default:
		ret = -EINVAL;
	}
...
static int adv7611_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case ADV7611_V4L2_CONTROL_OUTPUT_FORMAT:
		...
		channel->output_format = ctrl->value;
...
static int adv7611_querystd(struct v4l2_subdev *sd, v4l2_std_id *id)
{
	...
	if ( !gotformat ) {
		v4l2_dbg(1, debug, sd, "querystd: No std detected\n" );
		return -EINVAL;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	sd = &core->sd;
	v4l2_i2c_subdev_init(sd, client, &adv7611_ops);
	v4l_info(client, "chip found @ 0x%02x (%s)\n",
		 client->addr << 1, client->adapter->name);

	core->IO_ADDR = client->addr;
	if ( core->IO_ADDR == 0x4c ) {
		...
		ret = -EINVAL;
		v4l2_err(client, "adv7611 invalid I2C address %02x\n", 
			 client->addr );
		kfree(core);
		return ret;
	}
	...
	if ( ret != 0 ) {
		v4l2_err(client, "adv7611 init failed, code %d\n", ret );
		kfree(core);
		return ret;
	}
